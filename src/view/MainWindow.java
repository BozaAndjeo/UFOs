...
public class MainWindow extends javax.swing.JFrame {
    ...
    JLabel bbt; // JLabel of big, arrow controled, ship
    
    LinkedList <Bullet> bullets=new LinkedList <Bullet>(); // List of existent Bullet objects
    LinkedList <JLabel> bulletLabels=new LinkedList <JLabel>(); // List of JLabels for Bullets
    ...
    double bigPictureWidth=5200; // Actual width of picture that is used for background
    ...
    // As background is moving with cursor movement, it needs to have dimensions
    ...
    JLabel[][] field=new JLabel[100][100];
    
    ImageIcon bck=new ImageIcon();
    ImageIcon til=new ImageIcon();
    ImageIcon rtil=new ImageIcon();
    ImageIcon spc=new ImageIcon();
    ImageIcon lsr=new ImageIcon();
    ImageIcon exp=new ImageIcon();
    ImageIcon blt=new ImageIcon();
    
    LinkedList <Ship> spaceShip=new LinkedList <Ship>();
    LinkedList <JLabel> imageShip=new LinkedList <JLabel>();
    ...
    private void formMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_formMouseClicked
        ...
        //Point point=bbt.getMousePosition();
    ...
    synchronized public void activate(){
        ...
        blueTile = blueTileBuff;
        
        //Loading of Ship2
        ship2Buff=null;
        try
        {
            ship2Buff = ImageIO.read(getClass().getResource("/images/ship2.png"));
        } catch (Exception e) {}
        ship2 = ship2Buff;
        
        ImageIcon bigIcon=new ImageIcon(ship2); 
        bbt=new JLabel();
        bbt.setIcon(bigIcon);
        bbt.setSize(ship2Buff.getWidth(),ship2Buff.getHeight());
        layered.add(bbt);
    ...
    public synchronized void move(int x1, int y1, int x2, int y2){
        ...
        for(int i=0; i<t; i++){
            ...
            imageShip.get(selectedShip).setLocation(x1+dx, y1+dy);
        ...
        imageShip.get(selectedShip).setLocation((x2/50)*50, (y2/50)*50);
        spaceShip.get(selectedShip).x=(x2/50)*50;
        spaceShip.get(selectedShip).y=(y2/50)*50;
    ...
    public void mouseClicked(){
        if(!caught){
            ...
            if(spaceShip.get(i).x==(p.x/50)*50)
                if(spaceShip.get(i).y==(p.y/50)*50){
                    caught=true;
                    selectedX=spaceShip.get(i).x/50;
                    selectedY=spaceShip.get(i).y/50;
                    selectedShip=i;
            }
        }
        else{
            ...
            empty=true;
            for(int i=0; i<shipNumber; i++)
                if(spaceShip.get(i).x/50==p.x/50)
                    if(spaceShip.get(i).y/50==p.y/50)
                        empty=false;
            ...
            if(p.x/50<screenWidth/50 & p.y/50<screenHeight/50)
            {
                if(empty)
                    {                     
                        field[selectedX][selectedY].setIcon(til);
    ...
    public void mouseMoved(){
        p=getMousePosition();
        g.drawImage(moveableBackground, (int) (-p.x*0.1), (int) (-p.y*0.1), null);
                
        if(p.x/50<screenWidth/50 & p.y/50<screenHeight/50) {
            if(caught==true & actionInProgress==false) field[selectedX][selectedY].setIcon(rtil);
            field[p.x/50][p.y/50].setIcon(rtil);

            if ((p.x/50)!=lastX || (p.y/50)!=lastY)
            {
                field[lastX][lastY].setIcon(til);
                field[p.x/50][p.y/50].setIcon(rtil);
            }
            lastX=p.x/50;
            lastY=p.y/50;
...
public void placeShips(){
    for(int i=0; i<screenWidth/50; i++){
            ...
            imageShip.add(e);
            imageShip.get(i).setLocation(spaceShip.get(i).x, spaceShip.get(i).y);
    ...
    @Override
    public void run(){
        move(spaceShip.get(selectedShip).x,spaceShip.get(selectedShip).y,(p.x/50)*50, (p.y/50)*50);    
    }
    }
    ...
    synchronized public void drawLaser(){
                lc=new LaserCount();
                lc.start();
    }
    ...
    public class BoomCount extends Thread{
        ...
        synchronized public void run(){
                ...
                do{    
                    ...
                    g3.drawImage(explosion, 0, 0, rootPane);
                    //g3.rotate(1);
                    ...
                    explosion2=explosionBuff;
                    exp=new ImageIcon(explosion2);
                    
                    explosionLabel.setIcon(exp);
    ...
    public void fullScreen(){
        ...
        */
        ...
        moveableBackground = bigPictureBuff;
        
        System.out.println("Moveable Background Test: "+moveableBackground.getWidth(null));
        ...
        while(!exit){
            ...
            if(!(pressedKeyCodes.isEmpty())) do{
                ...
                if(pressedKeyCodes.contains("38")&!(pressedKeyCodes.contains("40"))) {
                    bbt.setLocation(bbt.getLocation().x, bbt.getLocation().y-5);
                    impulsY=-5;
                }   
                ...
                if(pressedKeyCodes.contains("37")&!(pressedKeyCodes.contains("39"))) {
                    ...
                    bbt.setLocation(bbt.getLocation().x-5, bbt.getLocation().y);
                    impulsX=-5;
                    tx2 = AffineTransform.getRotateInstance(u, 79, 79);
                    op2 = new AffineTransformOp(tx2, AffineTransformOp.TYPE_BILINEAR);
                    ship2=op2.filter(ship2Buff, null);
                    graphics=(Graphics2D) ship2.getGraphics();
                    //bbt.setSize(ship2.getWidth(null), ship2.getHeight(null));
                    ship2Icon=new ImageIcon(ship2);
                    bbt.setIcon(ship2Icon);
                    //bbt.setSize(ship2.getWidth(null), ship2.getHeight(null));
                }
                ...
                if(pressedKeyCodes.contains("32")){
                    ...
                    switch(intU){
                        case -5: 
                            bullet=new Bullet(u, bbt.getLocation().x+18, bbt.getLocation().y+75, impulsX, impulsY);
                            break;
                        case -4: 
                            bullet=new Bullet(u, bbt.getLocation().x+15, bbt.getLocation().y+71, impulsX, impulsY);
                            break;
                        case -3: 
                            bullet=new Bullet(u, bbt.getLocation().x+18, bbt.getLocation().y+68, impulsX, impulsY);
                            break;
                        case -2: 
                            bullet=new Bullet(u, bbt.getLocation().x+16, bbt.getLocation().y+64, impulsX, impulsY);
                            break;
                        case -1: 
                            bullet=new Bullet(u, bbt.getLocation().x+16, bbt.getLocation().y+61, impulsX, impulsY);
                            break;
                        case 0: 
                            bullet=new Bullet(u, bbt.getLocation().x+20, bbt.getLocation().y+59, impulsX, impulsY);
                            break;
                        case 1: 
                            bullet=new Bullet(u, bbt.getLocation().x+21, bbt.getLocation().y+51, impulsX, impulsY);
                            break;
                        case 2:
                            bullet=new Bullet(u, bbt.getLocation().x+20, bbt.getLocation().y+43, impulsX, impulsY);
                            break;
                        case 3:
                            bullet=new Bullet(u, bbt.getLocation().x+27, bbt.getLocation().y+36, impulsX, impulsY);
                            break;
                        case 4:
                            bullet=new Bullet(u, bbt.getLocation().x+29, bbt.getLocation().y+28, impulsX, impulsY);
                            break;
                        case 5:
                            bullet=new Bullet(u, bbt.getLocation().x+35, bbt.getLocation().y+25, impulsX, impulsY);
                            break;
                        default:
        ...
        public synchronized void run(){
            do{ 
                ...
                for(int i=0; i<bullets.size(); i++){
                    bullets.get(i).positionDX=bullets.get(i).positionDX+bullets.get(i).iX/4.0-cos(bullets.get(i).angle)*15/4.0;
                    bullets.get(i).positionDY=bullets.get(i).positionDY+bullets.get(i).iY/4.0-sin(bullets.get(i).angle)*15/4.0;
                    
                    bullets.get(i).positionX=(int) bullets.get(i).positionDX;
                    bullets.get(i).positionY=(int) bullets.get(i).positionDY;
                    
                    bullets.get(i).label.setLocation(bullets.get(i).positionX, bullets.get(i).positionY);
                }
                for(int i=0; i<bullets.size(); i++){
                    if (bullets.get(i).label.getLocation().x<=-13
                      ||bullets.get(i).label.getLocation().y<=-13
                      ||bullets.get(i).label.getLocation().y>=screenHeight) {
                        
                        layered.remove(bullets.get(i).label);
                        bullets.remove(i);
                        i--;
                    }
                }
